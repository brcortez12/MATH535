...
def ZeroMean(X, zType='CFA'):
    ...
    row = np.zeros([M], dtype=X.dtype)
    col = np.zeros([1,N], dtype=X.dtype)
    cm=0
    
    # subtract mean from each color channel
    mu = []
    for j in range(1):
        mu.append(np.mean(X[:,:], axis=(0,1)))
        X[:,:] -= mu[j]
    
    for j in range(1):
        row[:] = np.mean(X[:,:],axis=1)
        col[0,:] = np.mean(X[:,:],axis=0)

    if zType=='col':
        for j in range(1): Y[:,:] = X[:,:] - np.tile(col[1,:],(M,1))
    elif zType=='row':
        for j in range(1): Y[:,:] = X[:,:] - np.tile(row[j],(N,1)).transpose()
    elif zType=='both':
        for j in range(1): Y[:,:] = X[:,:] - np.tile(col[0,:],(M,1))
        for j in range(1): Y[:,:] = X[:,:] - np.tile(row[:],(N,1)).transpose()# equal to Y = ZeroMean(X,'row'); Y = ZeroMean(Y,'col');
    elif zType=='CFA':
